...
class QuantumShipDetector:
    ...
    def run(self, verbose: bool = False):
        ...
        if verbose:
            print("Histograms for Rows and Columns Detection:")
        for i in range(self.board.size):
            ...
            if len(to_consider) > 0:
                ...
                ev_scores.append(num / den)

        if verbose:
            print("\n")
            print("Results for Columns Detection:")
        ...
        mean_ev = sum(ev_scores) / len(ev_scores) if len(ev_scores) > 0 else 0
